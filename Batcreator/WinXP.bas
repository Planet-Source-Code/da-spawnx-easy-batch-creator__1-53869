...
'Example CenterFormTop Me
   ...
   End With
End Sub
Public Sub PercentBar(Shape As Control, done As Integer, total As Variant)
On Error Resume Next
Dim X As Integer, preset As String, preset2 As String, bnt As String
preset$ = GetFromINI("mp3 control", "preset", App.Path & "\ini files\mc.ini")
preset2$ = GetFromINI("preset", "name", App.Path & "\presets\" & preset$ & ".ini")
bnt$ = GetFromINI("preset", "button", App.Path & "\presets\" & preset$ & ".ini")
X = done / total * Shape.Width
Shape.Line (0, 0)-(Shape.Width, Shape.Height), (&H8000000F), BF
Shape.Line (0, 0)-(X - 10, Shape.Height), ("" & bnt$ & ""), BF
...
'\\   this will simply kill doubles in a list box
    ...
    Next i
End Sub
Public Function KaiInputBox(Message As String, Title As String, Optional Default As String) As String
    'inputbox that stays ontop and still
    'gets the inputted text
    'ps: i'm a genius, i was up at freakin'
    '3am tryin' to figure out how to get the
    'msgbox'es to stay on top, and i got it!
    'you can't take that from me! it's mine!
    With frmMsg
        .txtType.text = "inputbox"
        ...
        Do
            DoEvents
        Loop Until strInput$ <> "NULL"
        KaiInputBox$ = strInput$
        strInput$ = "NULL"
    End With
End Function
